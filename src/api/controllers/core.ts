...
export async function getCredit(refreshToken: string) {
  const result = await request("GET", "/commerce/v1/benefits/credit_info", refreshToken, {
    headers: {
      Referer: "https://jimeng.jianying.com/ai-tool/image/generate"
    }
  });
  return {
    totalCredit: result.total_credits || 0,
    giftCredit: result.gift_credits || 0,
    purchaseCredit: result.purchase_credits || 0,
    vipCredit: result.vip_credits || 0
  };
}
